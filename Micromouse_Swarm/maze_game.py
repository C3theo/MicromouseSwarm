...
class Maze():
    ...
    def __init__(self, mazeLayer, mazeArray):
        ...
        # Draw Grid
        for y in xrange(16):
            pygame.draw.line(self.mLayer, (0, 0, 0, 255),
                             (0, y*20), (320, y*20), 3)  # horizontal
            for x in xrange(16):
                pygame.draw.line(self.mLayer, (0, 0, 0, 255),
                                 (x*20, 0), (x*20, 320), 3)  # vertical
    ...
    def clearWall(self):
        ...
        for self.way in self.compass.keys():
            if self.pathFree():
                if self.way == 'W':
                    # knock down East
                    pygame.draw.line(self.mLayer, (0, 0, 0, 0),
                                     (dx, dy+2), (dx, dy+18), 3)
                elif self.way == 'S':
                    # knock down North
                    pygame.draw.line(self.mLayer, (0, 0, 0, 0),
                                     (dx+2, dy+20), (dx+18, dy+20), 3)
                elif self.way == 'E':
                    # knock down West
                    pygame.draw.line(self.mLayer, (0, 0, 0, 0),
                                     (dx+20, dy+2), (dx+20, dy+18), 3)
                elif self.way == 'N':
                    # knock down South
                    pygame.draw.line(self.mLayer, (0, 0, 0, 0),
                                     (dx+2, dy), (dx+18, dy), 3)
